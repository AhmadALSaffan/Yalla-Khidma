package com.yallakhedma.app.presentation.theme

import androidx.compose.runtime.staticCompositionLocalOf
import androidx.compose.ui.unit.dp

data class Spacing(
    val xs: androidx.compose.ui.unit.Dp = 4.dp,
    val sm: androidx.compose.ui.unit.Dp = 8.dp,
    val md: androidx.compose.ui.unit.Dp = 12.dp,
    val lg: androidx.compose.ui.unit.Dp = 16.dp,
    val xl: androidx.compose.ui.unit.Dp = 20.dp,
    val xxl: androidx.compose.ui.unit.Dp = 24.dp,
    val xxxl: androidx.compose.ui.unit.Dp = 32.dp,
    val huge: androidx.compose.ui.unit.Dp = 40.dp,
    val giant: androidx.compose.ui.unit.Dp = 48.dp,
    val mega: androidx.compose.ui.unit.Dp = 64.dp,
    // semantic
    val screenHorizontal: androidx.compose.ui.unit.Dp = 16.dp,
    val screenTop: androidx.compose.ui.unit.Dp = 46.dp,
    val cardPadding: androidx.compose.ui.unit.Dp = 16.dp,
)

val LocalSpacing = staticCompositionLocalOf { Spacing() }
